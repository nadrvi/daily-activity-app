  ...
  return (
    <div className="min-h-screen bg-slate-100 p-10 font-sans text-slate-800">
      <div className="max-w-2xl mx-auto bg-white p-8 rounded-xl shadow-md">
        <h1 className="text-3xl font-bold text-blue-600 mb-6">Daily Noted Activity 📝</h1>
        
        {/* FORM INPUT JADWAL BARU */}
        <form onSubmit={handleSubmit} className="mb-8 bg-slate-50 p-4 rounded-lg border border-slate-200">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              >
                ...
                <option value="Pagi Menjelang Siang 📚">Pagi Menjelang Siang 📚</option>
            ...
            <div>
              <label className="block text-sm text-gray-600 mb-1">Kegiatan</label>
              <input 
                type="text" 
                value={activityName}
            ...
            <div>
              <label className="block text-sm text-gray-600 mb-1">Mulai (Waktu)</label>
              <input 
                type="text" 
                value={timeStart}
            ...
            <div>
              <label className="block text-sm text-gray-600 mb-1">Selesai (Waktu)</label>
              <input 
                type="text" 
                value={timeEnd}
        ...
        {/* DAFTAR JADWAL */}
        <div>
          <h2 className="text-xl font-bold border-b pb-2 mb-4">Daftar Kegiatan</h2>
          {jadwal.length === 0 ? (
            <p className="text-gray-500 text-center py-4">Belum ada jadwal nih, isi form di atas bro!</p>
          ) : (
            <ul className="space-y-4">
              {jadwal.map((item) => (
                <li key={item.id} className="flex justify-between items-center border-b pb-3">
                  <div>
                    <p className="font-semibold text-gray-800">{item.time_start} - {item.time_end}</p>
